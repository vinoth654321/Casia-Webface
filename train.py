...
if(params['dataset'] == 'MNIST'):
    ...
    from models.feret_model import Generator, Discriminator, DHead, QHead
...
# tensorboardX training results start
writer = SummaryWriter('results/{}'.format(params['dataset']))
# tensorboardX training results end
...
# tensorboard add the training images start
img_grid_real = vutils.make_grid(sample_batch[0].to(device)[ : 100], nrow=10, padding=2, normalize=True).cpu()
writer.add_image(params['dataset'] + ' Training Images', img_grid_real)
# writer_real.close()
# tensorboard add the training images end
...
for epoch in range(params['num_epochs']):
    ...
    for i, (data, _) in enumerate(dataloader, 0):
        ...
        # writer.add_graph(discriminator,(fake_data,))
        # writer_graph.close()
        probs_fake = netD(output2).view(-1)
        # writer.add_graph(netD, (output1,))
        # writer_graph.close()
        ...
        writer.add_scalars('losses_%d_{}'.format(params['dataset']),
                               {'G_losses:' : G_loss, 'D_losses': D_loss},
                               epoch)
        # writer_scalar.close()
        iters += 1
    ...
    img_grid_fake = vutils.make_grid(gen_data, nrow=10, padding=2, normalize=True)
    writer.add_image(params['dataset'] + ' Fake Images', img_grid_fake)
    # writer_fake.close()
    ...
    if((epoch+1) == 1 or (epoch+1) == params['num_epochs']/2):
        ...
        plt.close('all')
        # writer.add_figure('Epoch_%d {}'.format(params['dataset']),
